...
public final class FirstPartTasks {
    ...
    public static List<String> allTracksSorted(Stream<Album> albums) {
        return albums.flatMap((s) -> s.getTracks().stream().map(Track::getName))
                .sorted().collect(Collectors.toList());
    }

    // Список альбомов, в которых есть хотя бы один трек с рейтингом более 95, отсортированный по названию
    public static List<Album> sortedFavorites(Stream<Album> s) {
        return s.filter(album -> album.getTracks().stream().mapToInt(Track::getRating)
                .anyMatch((value) -> (value > 95))).sorted(Comparator.comparing(Album::getName))
    ...
    public static Map<Artist, List<String>> groupByArtistMapName(Stream<Album> albums) {
        return albums.collect(Collectors.groupingBy(Album::getArtist)).entrySet().stream()
                .map((e) -> new AbstractMap.SimpleEntry<>(e.getKey(), e.getValue().stream()
                        .map(Album::getName).collect(Collectors.toList()))).collect(
                        Collectors.toMap(
                                Map.Entry::getKey,
                                Map.Entry::getValue
                        ));
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        return albums.collect(Collectors.groupingBy(Album::getName)).values().stream()
                .filter(album -> album.size() > 1).count();
    }
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.sorted(Comparator.comparing(album -> -album.getTracks().stream()
                .mapToInt(Track::getRating).average().orElse(0d)))
                .collect(Collectors.toList());
    }
